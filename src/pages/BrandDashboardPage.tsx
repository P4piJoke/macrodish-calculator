...
import { getProductsByBrand, updateProduct, deleteProduct } from "../services/product.service";
...
export default function BrandDashboardPage() {
  ...
  useEffect(() => {
    if (!userProfile?.brandId) return;
    getProductsByBrand(userProfile.brandId)
      .then(setProducts)
  ...
  return (
    <PageWrapper>
      <div className="flex items-center justify-between mb-6">
        ...
        {/* Redirect to Product Dictionary to add products */}
        <Link
          to="/products"
      ...
      {!loading && products.length === 0 && (
        <div className="text-center py-20 border-2 border-dashed border-gray-200 rounded-2xl">
          <p className="text-gray-400 text-sm mb-3">No products under your brand yet.</p>
          <Link
            to="/products"
            className="text-green-600 font-medium text-sm hover:underline"
          >
            Go to Product Dictionary to add one →
          </Link>
        </div>
      )}
